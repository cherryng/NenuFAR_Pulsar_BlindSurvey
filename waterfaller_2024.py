...
Cherry Ng - 2024 modified to adapt to NenuFAR data
...
def waterfall(rawdatafile, start, duration, dm=None, nbins=None, nsub=None,\
    ...
    startsamp = start_bin
    N = nbinsextra
    startsub = int(startsamp/rawdatafile.nsamp_per_subint)
    skip = startsamp - (startsub*rawdatafile.nsamp_per_subint)
    endsub = int((startsamp+N)/rawdatafile.nsamp_per_subint)
    trunc = ((endsub+1)*rawdatafile.nsamp_per_subint) - (startsamp+N)
    print('CHECK isub range', startsub, endsub+1) 
    """    
    sdata = rawdatafile.fits['SUBINT'].data[0]['DATA'][:,:,:,0]
    print("sdata shape", sdata.shape)
#    shp = sdata.squeeze().shape
    shp = sdata.shape
    #shp = (1024,1,192)
    print('after squeeze', shp)
    data2 = np.asarray(sdata, dtype=np.float32)
    print('CHECK data2 shape before applying changes', np.shape(data2))
    isub=27
    #apply zero off -- this is just 0.0)
    data2 += rawdatafile.zero_off
    #apply scale and apply offst
#    data2 = data2.reshape((rawdatafile.nsamp_per_subint,  rawdatafile.npoln * rawdatafile.nchan))
    print('CHECK data2 shape line 152)',data2.shape)
    print('CHECK shape rawdatafile.get_scales(0)', rawdatafile.get_scales(0).shape )
    print('CHECK DAT_SCL', rawdatafile.fits['SUBINT'].data[0]['DAT_SCL'].reshape(rawdatafile.npoln * rawdatafile.nchan,))
#    print(rawdatafile.get_scales(isub)[0:5])

    data2 *= rawdatafile.get_scales(isub).reshape(rawdatafile.npoln * rawdatafile.nchan,)

    #    data2 += rawdatafile.get_offsets(isub)
    #apply weight
#    data2 = data2.reshape((rawdatafile.nsamp_per_subint * rawdatafile.npoln, rawdatafile.nchan))
#    data2 *= rawdatafile.get_weights(isub)

    #CHECK Polarization
    print('CHECK npol', rawdatafile.npoln, 'shp', shp)
    if rawdatafile.npoln > 1:
        data2 = data2.reshape((rawdatafile.nsamp_per_subint, rawdatafile.npoln, rawdatafile.nchan))
    else:
        data2 = data2.reshape((rawdatafile.nsamp_per_subint, 1, rawdatafile.nchan))
    print('CHECK data2 shape after pol', np.shape(data2))

    #data2 = data2.reshape((rawdatafile.nsamp_per_subint, rawdatafile.nchan))

#    if (len(shp)==3 and shp[1]==rawdatafile.npoln and
#        (rawdatafile.poln_order=="AABBCRCI" or rawdatafile.poln_order=="AABB")):
#        print('CHECK case 1')        
#        warnings.warn("Polarization is %s, averaging AA and BB"%rawdatafile.poln_order)
#        data2 = 0.5*(data2[:,0,:] + data2[:,1,:])
#    elif (len(shp)==3 and shp[1]==rawdatafile.npoln and
#          rawdatafile.poln_order=="IQUV"):
#        print('CHECK case 2')
#        warnings.warn("Polarization is IQUV, just using Stokes I")
#        data2 = data2[:,0,:]


    print('CHECK data2 shape', np.shape(data2))
    """
    print('-------Done debugging ----------------')
    data = rawdatafile.get_spectra(start_bin, nbinsextra)
    print('[waterfall] after get_spectra data shape', np.shape(data))
    ...
    print('[waterfall] after zerodm data', np.shape(data.data))
    # Subband data
    ...
    print('[waterfall] after dedisperse data', np.shape(data.data))
    # Downsample
    if downsamp>1:
...
def main():
    ...
    if fn.endswith(".fil"):
        ...
        #rawdatafile.nsamp_per_subint = 512
        #print('Hacked into filterbank, hard-coding the subint 512')
    ...
    print('CHECK data', np.shape(data))
