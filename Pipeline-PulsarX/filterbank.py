...
class FilterbankFile(object):
    def __init__(self, filfn, mode='readonly'):
        ...
        self.times = self.tsamp * np.arange(self.nspec)
    ...
    def get_timeslice(self, start, stop):
        ...
        return self.get_spectra(startspec, stopspec)

    def get_spectra(self, start=0, stop=-1):
        if stop == -1:
            stop = self.nspec
        else:
        ...
        self.filfile.seek(round(pos), os.SEEK_SET)
    ...
    def write_spectra(self, spectra, ispec):
        ...
        if 'r+' not in self.filfile.mode.lower():
            raise ValueError(
                "FilterbankFile object for '%s' is not writable." %
                self.filename)
        nspec, nchans = spectra.shape
        if nchans != self.nchans:
