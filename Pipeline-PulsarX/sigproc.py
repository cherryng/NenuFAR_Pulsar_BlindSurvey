...
ids_to_machine = dict(
    list(zip(list(machine_ids.values()), list(machine_ids.keys()))))
...
def read_hdr_val(filfile, stdout=False):
    ...
    if isinstance(paramname, bytes):  #2026_01 Cherry adding this to fix 2to3 conversion issue
        paramname = paramname.decode('ascii')
...
def prep_string(string):
    ...
    if isinstance(string, str):
        string = string.encode('ascii')  # or 'utf-8' if needed
